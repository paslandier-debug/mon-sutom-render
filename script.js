...
async function initializeGame() {
    ...
    // 1. Appel de la nouvelle API
    try {
        const response = await fetch('/api/premiere_lettre');
        const data = await response.json();
        
        // 2. Mettre à jour les paramètres du jeu
        let LONGUEUR_MOT = data.longueur;
	let PREMIERE_LETTRE = data.premiereLettre;
        
        // 3. Créer la grille (maintenant que WORD_LENGTH est connu)
        // initialiserJeu(); 

        // 4. Révéler la première lettre
        afficherPremiereLettre(data.premiereLettre, data.couleur);
        
        // 5. Écouter les entrées clavier
        gererSoumission();

    } catch (error) {
        console.error("Erreur d'initialisation du jeu via l'API :", error);
        alert("Impossible de démarrer le jeu. Veuillez vérifier la connexion au serveur.");
    }
}
